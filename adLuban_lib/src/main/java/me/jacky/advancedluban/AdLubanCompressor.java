...
import static me.jacky.advancedluban.Preconditions.checkNotNull;
...
class AdLubanCompressor {

    private static final String TAG = "Luban Compress";

    private final AdLubanBuilder mLuban;

    private ByteArrayOutputStream mByteArrayOutputStream;

    AdLubanCompressor(AdLubanBuilder luban) {
        mLuban = luban;
    }
    ...
    private File compressImage(int gear, File file) throws IOException {
        switch (gear) {
            case CompressGear.LUBAN_GEAR:
                return lubanCompress(file);
            case CompressGear.CUSTOM_GEAR:
                return customCompress(file);
            case CompressGear.FAST_GEAR:
                return fastCompress(file);
            default:
                return file;
        }
    }
    ...
    /**
     * 主要使用Luban的算法，提供了类似微信的压缩效果，适用于普通压缩，没有文件大小限制以及图片的宽高限制
     *
     * @param file
     * @return
     * @throws IOException
     */
    private File lubanCompress(@NonNull File file) throws IOException {
    ...
    /**
     * 极速压缩模式，压缩之后的图片分辨率小于 1280 x 720, 文件最后小于60Kb，
     * 特殊情况下，小于原图片的1/5，适用于快速压缩，不计较最终图片品质
     *
     * @param file
     * @return
     * @throws IOException
     */
    private File fastCompress(@NonNull File file) throws IOException {
        int minSize = 60;
        int longSide = 720;
        int shortSide = 1280;
